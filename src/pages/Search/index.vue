            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                <li :class="{ active: isOne }" @click="changeOrder('1')">
                  <a
                    >综合
                    <!-- <span v-show="isOne" class="iconfont" :class="{'icon-arrowup':isAsc,'icon-arrowdown':isDesc}"></span> -->
                    <span v-show="isOne"
                ...
                <li :class="{ active: isTwo }" @click="changeOrder('2')">
                  <a
                    >价格
                    <!-- <span v-show="isTwo" class="iconfont" :class="{'icon-arrowup':isAsc,'icon-arrowdown':isDesc}">箭头</span> -->
                    <span v-show="isTwo"
                      ><span v-show="isAsc">⬆</span
                      ><span v-show="isDesc">⬇</span></span
                    >
...
export default {
  ...
  methods: {
    // 向服务器发请求获取search模块数据（根据参数返回不同的数据进行展示）
    // 把这次请求封装成一个函数，需要调用的时候调用即可
    getData() {
      //先测试接口返回的数据格式
    ...
    removeCategoryName() {
      ...
      // 但是把相应的字段变为undefined，当前这个字段就不会带给服务器
    ...
    changeOrder(flag) {
      ...
      // console.log('起始的flag'+originFlag,'用户点击的flag'+flag);
      // 准备一个新的order属性
      let newOrder = "";
      // 点击的是综合
      if (flag == originFlag) {
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      // 再次发起ajax请求
      this.getData();
      // 每一次请求完毕，应该把相应的1、2、3级分类的id置空，让它接受下一次的相应1,2,3级id
